...
**说明**:
- NGINX地址配置已从数据库表中移除，改为通过配置文件管理（application.yml中的nginx.config.remote配置）
- 网关实例的负载均衡配置通过gateway_group_detail表的detail_weight字段管理
...
| mode                      | varchar(20)  | NOT NULL | DISTRIBUTED       | 限流模式：DISTRIBUTED(分布式)、LOCAL_DISTRIBUTED(本地+分布式混合) |
| local_batch_size          | int          | NOT NULL | 100               | 本地批量获取令牌数（仅在LOCAL_DISTRIBUTED模式下使用）        |
| local_capacity_multiplier | double       | NOT NULL | 1.0               | 本地容量倍数（仅在LOCAL_DISTRIBUTED模式下使用）              |
| create_time               | datetime     | NOT NULL | CURRENT_TIMESTAMP | 创建时间                                                     |
| update_time               | datetime     | NOT NULL | CURRENT_TIMESTAMP | 更新时间（自动更新）                                         |
...
- 支持两种限流模式：纯分布式（DISTRIBUTED）、本地+分布式混合（LOCAL_DISTRIBUTED）
- 配置变更实时推送到所有网关节点
