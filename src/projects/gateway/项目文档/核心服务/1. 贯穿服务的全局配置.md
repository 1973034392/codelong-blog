...
2. `RateLimitHandler`：分布式限流检查（全局、服务、接口、IP四级限流）
3. `PreExecutorHandler`：执行请求前的自定义逻辑（黑白名单等）
4. `ExecutorHandler`：核心业务处理（路由转发,泛化调用等）
5. `PostExecutorHandler`：响应后处理（日志记录、结果分析等）
6. `ResultHandler`：最终响应结果组装
