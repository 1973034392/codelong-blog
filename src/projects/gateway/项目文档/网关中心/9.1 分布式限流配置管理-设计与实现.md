...
CREATE TABLE `gateway_rate_limit` (
    ...
    `mode` VARCHAR(20) NOT NULL DEFAULT 'DISTRIBUTED' COMMENT '限流模式：DISTRIBUTED(分布式)/LOCAL_DISTRIBUTED(本地+分布式混合)',
    `local_batch_size` INT NOT NULL DEFAULT 100 COMMENT '本地批量获取令牌数（仅在LOCAL_DISTRIBUTED模式下使用）',
    `local_capacity_multiplier` DOUBLE NOT NULL DEFAULT 1.0 COMMENT '本地容量倍数（仅在LOCAL_DISTRIBUTED模式下使用）',
...
| `mode` | VARCHAR(20) | 限流模式 | DISTRIBUTED / LOCAL_DISTRIBUTED |
| `local_batch_size` | INT | 本地批量大小 | 100（默认值） |
| `local_capacity_multiplier` | DOUBLE | 本地容量倍数 | 1.0（默认值） |
...
private void syncConfigToRedis(GatewayRateLimitDO config) {
    ...
    if (config.getLocalBatchSize() != null) {
        configMap.put("localBatchSize", config.getLocalBatchSize());
    }
    if (config.getLocalCapacityMultiplier() != null) {
        configMap.put("localCapacityMultiplier", config.getLocalCapacityMultiplier());
    }
...
private void publishConfigUpdate(GatewayRateLimitDO config) {
    ...
    // 发布新增的配置字段
    if (config.getMode() != null) {
        message.put("mode", config.getMode());
    }
    if (config.getLocalBatchSize() != null) {
        message.put("localBatchSize", config.getLocalBatchSize());
    }
    if (config.getLocalCapacityMultiplier() != null) {
        message.put("localCapacityMultiplier", config.getLocalCapacityMultiplier());
    }
