...
下表将InnoDB的`ReadView`可见性检查逻辑进行了归纳。
...
在此隔离级别下，事务中的*每一条语句*开始执行时，都会创建一个新的快照 。这意味着，一个事务可以看到在它执行期间，由其他事务提交的更改。这种策略可以防止“脏读”（读取未提交的数据），但允许“不可重复读”（在同一事务中两次读取同一行数据，得到不同结果）。
